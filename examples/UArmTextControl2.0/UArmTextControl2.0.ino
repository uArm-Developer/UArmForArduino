...
String runCommand(String cmnd){
    ...
    if(cmnd.indexOf(F("attach")) >= 0){
      ...
      if(values[0] == 1){           uarm.g_servo_rot.attach(11);
      }else if(values[0] == 2){    uarm.g_servo_left.attach(13);
      }else if(values[0] == 3){   uarm.g_servo_right.attach(12);
      }else if(values[0] == 4){uarm.g_servo_hand_rot.attach(10);
      }else{ return F("[ERROR: Servo number does not exist]\n");}
      return ok + cmnd + endB;
    }
    ...
    if(cmnd.indexOf(F("detach")) >= 0){
      ...
      float values[1];
      getCommandValues(cmnd, attachDetachParameters, 1, values);
      if(values[0] == 1){      uarm.g_servo_rot.detach();
      }else if(values[0] == 2){uarm.g_servo_left.detach();
      }else if(values[0] == 3){uarm.g_servo_right.detach();
      }else if(values[0] == 4){uarm.g_servo_hand_rot.detach();
      }else{ return "[ERROR: Servo number " + String(values[0]) + " does not exist]\n";}
      return ok + cmnd + endB;
    }
